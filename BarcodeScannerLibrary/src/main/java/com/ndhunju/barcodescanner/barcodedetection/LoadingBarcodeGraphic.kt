...
/**
 * This class extends [BarcodeGraphicBase] to draw the graphic to indicate the barcode result is
 * in loading.
 */
class LoadingBarcodeGraphic(
    ...
    override fun draw(canvas: Canvas) {
        ...
        val path = Path()
        // The distance between the box's left-top corner and the starting point of white path.
        ...
        while (i < 4) {
            ...
            if (offsetLen <= edgeLen) {
                lastPathPoint.x = boxClockwiseCoordinates[i].x +
                        coordinateOffsetBits[i].x *
                        offsetLen
                lastPathPoint.y = boxClockwiseCoordinates[i].y +
                        coordinateOffsetBits[i].y *
                        offsetLen

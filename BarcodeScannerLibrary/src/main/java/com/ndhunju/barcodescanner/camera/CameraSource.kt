...
class CameraSource(private val graphicOverlayView: GraphicOverlayView) {
    ...
    private val context: Context = graphicOverlayView.context
    ...
    private fun setPreviewAndPictureSize(camera: Camera, parameters: Parameters) {
        ...
        val sizePair: CameraSizePair = Settings.getPreviewSize() ?: run {
            ...
            val displayAspectRatioInLandscape: Float =
                if (graphicOverlayView.context.isInPortraitMode()) {
                    graphicOverlayView.height.toFloat() / graphicOverlayView.width
                } else {
                    graphicOverlayView.width.toFloat() / graphicOverlayView.height
                }
        ...
        previewSize = sizePair.preview.also {
            ...
            Settings.setPreviewSize(it)
        }

        sizePair.picture?.let { pictureSize ->
            Log.v(TAG, "Camera picture size: $pictureSize")
            parameters.setPictureSize(pictureSize.width, pictureSize.height)
            Settings.setPictureSize(pictureSize)
        }
    }
    ...
    private fun createPreviewBuffer(previewSize: Size): ByteArray {
        ...
        val sizeInBits = previewSize.height.toLong() *
                previewSize.width.toLong() *
                bitsPerPixel.toLong()
        ...
        check(!(!byteBuffer.hasArray() || !byteBuffer.array().contentEquals(byteArray))) {
            // This should never happen. If it does, then we wouldn't be passing the preview
            // content to the underlying detector later.
    ...
    private inner class FrameProcessingRunnable constructor() : Runnable {
         ...
         * run detection on that frame. It immediately loops back for the next frame without pausing.
         *
         *
            ...
            while (true) {
                synchronized(lock) {
                    while (active && pendingFrameData == null) {
                        try {
                            // Wait for the next frame to be received from the camera,
                            // since we don't have it yet.
                            lock.wait()
                        } catch (e: InterruptedException) {
                    ...
                    if (!active) {
                        ...
                        // We check this here, immediately after the wait() above, to handle the
                        // case where setActive(false) had been called, triggering the termination
                        // of this loop.
                        return
                    }
                ...
                try {
                    synchronized(processorLock) {
                        previewSize?.let { previewSize ->
                            ...
                            data?.let {
                                frameProcessor?.process(it, frameMetadata, graphicOverlayView)
                            }
                        }
                    }
                } catch (t: Exception) {
    ...
    companion object {
         ...
         * [ ][.MAX_CAMERA_PREVIEW_WIDTH]]. If there're multiple candidates,
         * choose the one having longest width.
         *
         *
         ...
         * preview size and the picture size of the camera together, because these need to have
         * the same aspect ratio. On some hardware, if you would only set the preview size, you
         * will get a distorted image.
         *
        ...
        private fun selectSizePair(
            ...
            if (selectedPair == null) {
                // Picks the one that has the minimum sum of the differences between the
                // desired values and the actual values for width and height.
        ...
        private fun selectPreviewFpsRange(camera: Camera): IntArray? {
            ...
            // For example, if the desired frame rate is 29.97, the range (30, 30) is probably
            // more desirable than the range (15, 30).
         ...
         * no corresponding picture size of the same aspect ratio. If there is a corresponding
         ...
         */
        private fun generateValidPreviewSizeList(camera: Camera): List<CameraSizePair> {
            val parameters = camera.parameters
            ...
            for (previewSize in supportedPreviewSizes) {
                ...
                for (pictureSize in supportedPictureSizes) {
                    val pictureAspectRatio = pictureSize.width.toFloat()
                        .div(pictureSize.height.toFloat())
            ...
            // allow all of the preview sizes and hope that the camera can handle it.
            // Probably unlikely, but we still account for it.
